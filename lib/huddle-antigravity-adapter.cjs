...
process.stdin.on('end', () => {
  ...
  const run = (script, timeout = 9000) => { try { return execFileSync('bash', [path.join(here, script)], { cwd, input: payload, encoding: 'utf8', timeout }); } catch { return ''; } };
  let output = phase === 'post-tool' ? '' : start ? run('huddle-session-start.sh', 15000) : run('huddle-poll.sh', 9000);
  if (phase === 'post-tool') {
